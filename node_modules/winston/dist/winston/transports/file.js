  ...
  return _createClass(File, [{
    ...
    value: function log(info) {
      ...
      if (this._drain) {
        this._stream.once('drain', function () {
          _this4._drain = false;
          _this4.log(info, callback);
        });
        return;
      }
      if (this._rotate) {
        this._stream.once('rotate', function () {
          _this4._rotate = false;
          _this4.log(info, callback);
        });
        return;
      }
    ...
    value: function open() {
      ...
      this.stat(function (err, size) {
        ...
        debug('stat done: %s { size: %s }', _this6.filename, size);
        _this6._size = size;
        _this6._dest = _this6._createStream(_this6._stream);
        _this6._opening = false;
        _this6.once('open', function () {
          if (!_this6._stream.emit('rotate')) {
            _this6._rotate = false;
          }
        });
      });
    }
    ...
    value: function stat(callback) {
      ...
      fs.stat(fullpath, function (err, stat) {
        ...
        if (!stat || _this7._needsNewFile(stat.size)) {
          // If `stats.size` is greater than the `maxsize` for this
          // instance then try again.
          return _this7._incFile(function () {
            return _this7.stat(callback);
          });
        }
    ...
    value: function _endStream() {
      ...
      if (this._dest) {
        ...
        this._dest.end(function () {
          _this0._cleanupStream(_this0._dest);
          callback();
        });
      } else {
        callback(); // eslint-disable-line callback-return
    ...
    value: function _createStream(source) {
      ...
      .on('error', function (err) {
        ...
        _this1.emit('open', fullpath);
        source.pipe(dest);

        // If rotation occured during the open operation then we immediately
        // start writing to a new PassThrough, begin opening the next file
        // and cleanup the previous source and dest once the source has drained.
        if (_this1.rotatedWhileOpening) {
          _this1._stream = new PassThrough();
          _this1._stream.setMaxListeners(30);
          _this1._rotateFile();
          _this1.rotatedWhileOpening = false;
          _this1._cleanupStream(dest);
          source.end();
        }
      });
      ...
      for (var x = this.maxFiles - 1; x > 1; x--) {
        tasks.push(function (i, cb) {
          ...
          fs.exists(tmppath, function (exists) {
            ...
            fs.rename(tmppath, path.join(_this10.dirname, fileName), cb);
          });
        }.bind(this, x));
      }
      asyncSeries(tasks, function () {
        fs.rename(path.join(_this11.dirname, "".concat(basename).concat(ext).concat(isZipped)), path.join(_this11.dirname, "".concat(basename, "1").concat(ext).concat(isZipped)), callback);
      });
    }
